...
export async function GET(req: Request) {
    ...
    count = (await redis.incr(key)) ?? 0;

    if (badge) {
        return generateSVGBadge(String(count), badgeContent, badgeColor, badgeLabelColor, badgeStyle);
    }

    return new NextResponse(JSON.stringify({ code: 200, type, input, count, message: 'count incremented', staticBadge: `/api/badge?content=${badgeContent}&data=${count}` }), {
        status: 200,
